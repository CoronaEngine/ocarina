...
[[nodiscard]] OC_DEVICE_FLAG inline auto oc_face_forward(oc_float3 n, oc_float3 i, oc_float3 n_ref) noexcept { return oc_dot(n_ref, i) < 0.0f ? n : -n; }

[[nodiscard]] OC_DEVICE_FLAG inline auto oc_face_forward(oc_float3 v1, oc_float3 v2) noexcept { return oc_dot(v1, v2) > 0 ? v1 : -v1; }

[[nodiscard]] OC_DEVICE_FLAG inline auto oc_determinant(const oc_float2x2 m) noexcept {
    return m[0][0] * m[1][1] - m[1][0] * m[0][1];
}

[[nodiscard]] OC_DEVICE_FLAG inline auto oc_determinant(const oc_float3x3 m) noexcept {// from GLM
    return m[0].x * (m[1].y * m[2].z - m[2].y * m[1].z) - m[1].x * (m[0].y * m[2].z - m[2].y * m[0].z) + m[2].x * (m[0].y * m[1].z - m[1].y * m[0].z);
}

[[nodiscard]] OC_DEVICE_FLAG inline auto oc_determinant(const oc_float4x4 m) noexcept {// from GLM
...
[[nodiscard]] OC_DEVICE_FLAG inline auto oc_inverse(const oc_float2x2 m) noexcept {
...
[[nodiscard]] OC_DEVICE_FLAG inline auto oc_inverse(oc_float3x3 m) noexcept {// from GLM
...
[[nodiscard]] OC_DEVICE_FLAG inline auto oc_inverse(const oc_float4x4 m) noexcept {// from GLM
    const auto coef00 = m[2].z * m[3].w - m[3].z * m[2].w;
    const auto coef02 = m[1].z * m[3].w - m[3].z * m[1].w;
    const auto coef03 = m[1].z * m[2].w - m[2].z * m[1].w;
    const auto coef04 = m[2].y * m[3].w - m[3].y * m[2].w;
...
OC_DEVICE_FLAG inline void oc_coordinate_system(oc_float3 v1, oc_float3 &v2, oc_float3 &v3) noexcept {
...
OC_DEVICE_FLAG inline void oc_make_normal_tangent(oc_float3 N, oc_float3 T, oc_float3 &a, oc_float3 &b){
